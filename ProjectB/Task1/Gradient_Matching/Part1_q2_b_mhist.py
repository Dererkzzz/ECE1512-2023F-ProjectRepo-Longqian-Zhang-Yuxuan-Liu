...
from utils import MHISTDataset
...
def main():
    ...
    mean, std = calculate_mean_std(dst_train)
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
            	for model_eval in model_eval_pool:
                    ...
                    accs = []
                    # args.num_eval
                    for it_eval in range(100): # change 100
    ...
    mean = [0.7383, 0.6490, 0.7757] 
    std = [0.1648, 0.2166, 0.1357]
    
    result_path = os.path.join(args.save_path, 'MHIST_result_noise_conv6_it200')
    ssa_path = os.path.join(result_path, 'ssa')
    hp_path = os.path.join(result_path, 'HP')

    os.makedirs(result_path, exist_ok=True)
    os.makedirs(ssa_path, exist_ok=True)
    ...
    for i in range(len(image_syn)):
        ...
        if label == 0:  # 假设标签 0 代表 ssa 类
            save_path = os.path.join(ssa_path, f'image_{i}.png')
        else:  # 假设标签 1 代表 HP 类
            save_path = os.path.join(hp_path, f'image_{i}.png')
    ...
    # 显示图像网格
    ax.imshow(dataset_syn)

    # 添加类别标签
    num_rows = image_syn.shape[0] // 10  # 假设每行有25张图像
    row_height = dataset_syn.shape[0] / num_rows
    for i in range(num_rows):
        label = "ssa" if i < 5 else "hp"  # 前两行为 ssa，后两行为 hp
        ax.text(0, i * row_height, label, color='red', fontsize=100, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.5))

    plt.savefig('MHIST_syn_conv6_200it_noise_labeled.jpg', dpi=300)

    #plt.figure(dpi=300,figsize=(4,4))
    #plt.axis('off')
    #plt.imshow(dataset_syn)

    #plt.savefig('MHIST_syn_conv6_100it_noise.jpg',dpi=300)
