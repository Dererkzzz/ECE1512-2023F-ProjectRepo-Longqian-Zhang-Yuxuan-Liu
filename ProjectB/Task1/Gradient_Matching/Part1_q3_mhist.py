...
from networks import ConvNet, VGG11BN,AlexNet,AlexNetBN, ResNet18BN_AP,VGG11, VGG
from utils import get_dataset
import matplotlib.pyplot as plt
import numpy as np
import utils
from utils import MHISTDataset,get_network
...
train_dataset = MHISTDataset_train(root_dir='./result/MHIST_result_real', transform=transform)
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
...
# 初始化模型、损失函数和优化器
#model = ConvNet(channel=3, num_classes=2, net_width=128, net_depth=7, net_act='relu', net_norm='instancenorm', net_pooling='avgpooling', im_size=(224, 224))




#model = ResNet18BN_AP(channel=3, num_classes=2)
#model = VGG(vgg_name='VGG11',channel=3, num_classes=2, norm='instancenorm')
model = AlexNet(3, 2)
