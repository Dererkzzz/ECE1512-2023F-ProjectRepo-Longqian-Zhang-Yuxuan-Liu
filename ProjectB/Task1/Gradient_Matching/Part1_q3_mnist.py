...
import os
from networks import ConvNet, VGG11, AlexNet
from utils import get_dataset
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(image_path).convert('L')  # 将图像转换为灰度格式
...
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.1307,), (0.3081,))  # 标准化，这里使用了MNIST数据集的均值和标准差
])
...
train_dataset = CustomDataset(root_dir='./result/MNIST_result_real', transform=transform)
