...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        dst_train = MHISTDataset(csv_file=os.path.join(data_path, 'mhist_dataset/annotations.csv'), 
                                 root_dir=os.path.join(data_path, 'mhist_dataset/images/images'), 
                                 partition='train', 
                                 transform=transform)
        dst_test = MHISTDataset(csv_file=os.path.join(data_path, 'mhist_dataset/annotations.csv'), 
                                root_dir=os.path.join(data_path, 'mhist_dataset/images/images'), 
                                partition='test', 
                                transform=transform)
        class_names = ['SSA', 'HP']  # 根据MHIST的实际类别名称调整
...
def get_network(model, channel, num_classes, im_size=(224, 224)):
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    return net, acc_train, acc_test, precision, recall, f1
