...
def parse_arguments():
    ...
    parser.add_argument('--exp_name', '-name', type=str, default='mnist', help='set up path for storing the results')
    ...
    parser.add_argument('--spc', type=int, default=10, help='sample(s) per class')
    ...
    parser.add_argument('--init_method', type=str, default='real', choices=['real', 'noise'], help='initialization method for synthetic data')
...
def main():
    ...
    for exp in range(args.num_exp):
        ...
        if args.load_checkpoint or args.only_eval:
            ...
            image_syn = checkpoint['image_syn'].to(args.device)
            label_syn = checkpoint['label_syn'].to(args.device)
            print('iterstart: {}'.format(iter_start))
        else:
            # 根据 init_method 参数选择初始化方法
            if args.init_method == 'real':
                # 从真实数据初始化合成数据
                image_syn = torch.empty(size=(num_classes * args.spc, channel, im_size[0], im_size[1]), dtype=torch.float, device=args.device)
                for c in range(num_classes):
                    indices = np.random.choice(indices_class[c], args.spc, replace=False)
                    image_syn[c*args.spc:(c+1)*args.spc] = images_all[indices]
        ...
        label_syn = torch.tensor([np.ones(args.spc) * i for i in range(num_classes)], dtype=torch.long, requires_grad=False, device=args.device).view(-1)
        ...
        for it in range(iter_start, args.iteration + 1):  ## re-initialize the model for each iter
            ...
            if it == args.iteration:  ## record the final results
                ...
                print('\nSaving synthesized images at the last iteration...\n')
                # Create a directory to save class-wise images
                image_save_dir = os.path.join(save_path, 'synthesized_images')
                mkdir(image_save_dir)

                # Iterate over synthesized images
                for i in range(image_syn.shape[0]):
                    class_label = label_syn[i].item()  # Get the class label
                    class_dir = os.path.join(image_save_dir, str(class_label))  # Directory for the class
                    mkdir(class_dir)  # Create the directory if it doesn't exist

                    # Save the image
                    img = image_syn[i].cpu().detach()
                    img_save_path = os.path.join(class_dir, f'image_{i}.png')
                    save_image(img, img_save_path)

                print('Synthesized images saved successfully.')
