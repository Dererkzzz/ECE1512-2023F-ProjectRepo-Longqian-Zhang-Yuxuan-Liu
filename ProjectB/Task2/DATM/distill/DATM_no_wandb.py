...
from utils.utils_baseline import get_network, get_eval_pool, evaluate_synset, get_time, DiffAugment, ParamDiffAug
from utils.utils_gsam import get_dataset
...
def main(args):
    ...
    # First, create a list of numpy arrays
    numpy_arrays = [np.ones(args.ipc) * i for i in range(num_classes)]

    # Convert the list of numpy arrays to a single numpy array
    combined_numpy_array = np.concatenate(numpy_arrays)

    # Now convert the numpy array to a PyTorch tensor
    label_syn = torch.tensor(combined_numpy_array, dtype=torch.long, requires_grad=False, device=args.device).view(-1)
    ...
    if args.dataset in ["MNIST"] and not args.zca:
        expert_dir += "_NO_ZCA"
    ...
    if args.pix_init == 'real':
        ...
        if cfg.Initialize_Label_With_Another_Model:
            ...
            # 首先检查CUDA是否可用，并据此设置设备
            #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

	    # 接着获取网络模型，并将其移动到指定的设备上
            #Temp_net = get_network(args.model, channel, num_classes, im_size, dist=False)
    ...
    for it in range(0, args.Iteration+1):
        ...
        print(it)
        if it == args.Iteration:  # 在最后一次迭代时
            print('Final iteration, saving images...')
            with torch.no_grad():
                # 保存图像网格
                nrow = args.ipc  # 每行的图像数量
                grid = torchvision.utils.make_grid(image_syn, nrow=nrow, normalize=True, padding=2)
                grid_filename = os.path.join(save_dir, "image_grid.png")
                torchvision.utils.save_image(grid, grid_filename)
            
                # 保存每个类别的图像到相应目录
                for i in range(len(image_syn)):
                    # Assuming label_syn[i] is a one-hot encoded tensor and you want the index of the max value
                    class_label = torch.argmax(label_syn[i]).item()
                    img_filename = os.path.join(save_dir, f"class_{class_label}", f"image_{it}_{i}.png")
                    torchvision.utils.save_image(image_syn[i], img_filename)
