...
def condense(args, logger, device='cuda'):
    ...
    for it in range(n_iter):
        ...
        if it == n_iter - 1:
            for c in range(nclass):
                # 创建以类别编号命名的目录
                class_dir = os.path.join(args.save_dir, 'result_10class', str(c))
                os.makedirs(class_dir, exist_ok=True)

                # 为每个类别生成并保存10张独立的图片
                for img_index in range(10):
                    img_syn, _ = synset.sample(c, max_size=1)  # 假设每次sample返回1张图片

                    # 保存每张图像到相应的目录
                    class_img_path = os.path.join(class_dir, f'img_class_{c}_{img_index}.png')
                    save_img(class_img_path, img_syn, unnormalize=False, dataname=args.dataset)
